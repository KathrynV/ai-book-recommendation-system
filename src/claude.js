...
Base your picks entirely on this description.`
  ...
  const system = `You are a book recommendation assistant for a bookstore and lending library.
${contextBlock}
...
- Call submit_recommendations exactly once with your final answer.`;
  ...
  // Catalog SKUs are bare numeric ISBNs, so a model will sometimes emit
  // sku/title as a JSON number rather than a string even though the schema
  // says "string" — coerce instead of rejecting, since rejecting on strict
  // typeof silently threw out every recommendation in testing.
...
export async function getCustomerSummary({ apiKey, customerName, historyCount, sampleTitles, topAuthors, topCategories }) {
  const system = `You are a librarian assistant for a bookstore and lending library.
