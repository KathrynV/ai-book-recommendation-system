...
// Shared by every endpoint. Falls back to ANTHROPIC_API_KEY from the
// environment when the request didn't supply one — a convenience for
// running this demo locally without retyping a key every time. In the
// real tool this backs, staff always paste their own key per request.
function resolveApiKey(bodyApiKey) {
  return bodyApiKey || process.env.ANTHROPIC_API_KEY || "";
}

// Returns an error string, or null if apiKey is OK.
function checkApiKey(apiKey) {
  if (!apiKey || typeof apiKey !== "string") {
    return "Anthropic API key is required (set it in the form, or ANTHROPIC_API_KEY in .env).";
  }
  // HTTP header values are restricted to Latin-1 bytes — sending anything
  // outside that range throws a cryptic low-level "ByteString" error from
  // fetch() rather than a useful message. Catch it here with a clear one.
...
server.listen(PORT, () => {
  console.log(`ai-book-recommendation-system listening on http://localhost:${PORT}`);
});
